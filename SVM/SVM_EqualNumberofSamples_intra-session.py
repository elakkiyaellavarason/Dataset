...
for user in range(104,150):
    ...
    #Vertical
    print("USERRRRRR:", user)
    Data = pd.read_csv('C:\\Users\\ee244\\Desktop\\PhD\\Analysis\\SVM\\Evaluation\\Session 1\\User' + str(user) + '\\df_Training_s2_session2.csv')
    Data_sitting_session2 = pd.read_csv('C:\\Users\\ee244\\Desktop\\PhD\\Analysis\\SVM\\Evaluation\\Session 1\\User' + str(user) + '\\df_Training_s2_session2.csv')
    ...
    random_user = random.randint(102, 148)
    if user == random_user:
 ...
 #   y_pred_s2 = svclassifier.predict(X_s2_test)
  ...
  #  svclassifier.fit(X_s3_train, y_s3_train)
   # y_pred_s3 = svclassifier.predict(X_s3_test)
    ...
    myData_s1 = [user,"Sitting Vs Sitting", roc_auc,eer,thresh]
    #myData_s2 = [user,2, s2_tn, s2_fp, s2_fn, s2_tp, accuracy_rate_s2]
    #myData_s3 = [user, 3, s3_tn, s3_fp, s3_fn, s3_tp, accuracy_rate_s3]


    myFile = open('C:\\Users\\ee244\\Desktop\\PhD\\Analysis\\SVM\\Evaluation\\Session 1\\session 1 results\\equal samples\\Probability_scores\\Horizontal_NumberofsamplesTraining40_SittingVsSittingSession2.csv', 'a')
    with myFile:
...
#plt.show()
plt.savefig('C:\\Users\\ee244\\Desktop\\PhD\\Analysis\\SVM\\Evaluation\\Session 1\\session 1 results\\equal samples\\ROC Curves\\Scenario 1\\Vertical\\ROC_Horizontal_NumberofsamplesTraining40_SittingVsSittingSession2.jpg',dpi = 100)